...
def most_goals(players_list: BallPlayer) -> str:
    most_goals_player = max(players_list, key=lambda player : player.goals)
    return most_goals_player.name

def most_points(players_list: BallPlayer) -> tuple:
    most_points_player = max(players_list, key=lambda player : player.goals + player.passes)
    return (most_points_player.name, most_points_player.number)

def least_minutes(players_list: BallPlayer) -> object:
    least_minutes_player = min(players_list, key=lambda player : player.minutes)
